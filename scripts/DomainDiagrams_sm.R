...
##
## Purpose of script: produce iToL annotation file with Pfam domains
##
...
option_list = list(
  ...
  make_option( c("-o", "--outfile") , type = "character" , default=NULL, 
               help="output iTOL txt file", metavar="character"),
  make_option( c("-f", "--fasta") , type = "character" , default=NULL, 
               help="full length sequences in fasta format", metavar="character"),
  make_option( c("-a", "--afa") , type = "character" , default=NULL, 
               help="aligned sequences in aligned fasta format", metavar="character")
); 
...
if (is.null(opt$o)){
  print_help(opt_parser)
  stop("Provide output file name with -o", call.=FALSE)
}

if (is.null(opt$fasta)){
  print_help(opt_parser)
  stop("Provide full length sequences with -f", call.=FALSE)
}

if (is.null(opt$afa)){
  print_help(opt_parser)
  stop("Provide aligned sequences with -a", call.=FALSE)
}
...
lrr_doms <- tibble(Dom = paste0("LRR_",1:12),
                   Shape = rep("RE",12),
                   Color = rep("#e31a1c",12))
main_doms <- rbind(main_doms,lrr_doms)
if (!file.exists("All_Domains_with_colors.tsv")){
...
}else{all_doms<-read_delim("All_Domains_with_colors.tsv",
                           delim = "\t", 
                           col_types = cols(
                                            Dom = col_character(),
                                            Shape = col_character(),
...
a<-readAAStringSet(opt$f)
prot_l <- tibble(Gene = str_trim(a@ranges@NAMES), Length = a@ranges@width)
